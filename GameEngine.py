...
def recycle_card(inp, recent_card, board_):
...
def try_card_recycle(parent, card_pos):
    ...
    parent.state[state_conv(card_pos[2], card_pos[3])] = 0
    for column in range(8):
        row = parent.track[column]
        for orient in range(1, 9):
            inp = card_pos + (orient, row, column)
            result = recycle_card(inp, parent.pos, copy.copy(parent.state))
            if result is None:
                continue
            tmp_track = copy.copy(parent.track)
            if card_number % 2 == 0:
